...
class PieChart < Phlex::HTML
  ...
  def initialize(label:, values:, horizontal: true, radius: "5rem", **attrs)
    ...
    @id         = attrs.delete(:id) || label&.downcase&.underscore || "piechart-#{Time.now.to_i}"
    @radius     = radius
    @attrs      = attrs
  end
  ...
  def view_template
    ...
    div(**classes("piechart-container", horizontal?: "horizontal")) do
      div(**classes("piechart #{chart_classes}"), style: "#{pie_style};#{chart_styles}")
      if @label.present?
        label(**classes(label_classes), style: label_styles) { @label }
      end
    end
  ...
  def app_styles
    style do
      unsafe_raw <<~CSS
        .piechart-container {
          display: grid;
          grid-template-columns: #{@radius};
          gap: 3px;
        }
        .piechart-container.horizontal {
          grid-template-columns: #{@radius} #{@radius};
        }
        .piechart-container > label {
          margin: auto;
        }
        .piechart {
          border-radius: 50%;
  ...
  def pie_style
    ...
    bg = "background: conic-gradient(#{percentages.join(', ')})"
    "#{bg}; width: #{@radius}; height: #{@radius};"
  end
end
